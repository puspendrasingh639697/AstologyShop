...
// ✅ Backend se aane wali 9 categories ke exact naam
const CATEGORY_NAMES = {
  FESTIVAL: 'Festival Collections',
  DAILY_PUJA: 'Daily Puja Essentials',
  GEMSTONES: 'Gemstones & Rudraksha',
  YANTRA_IDOLS: 'Yantra & Idols',
  PUJA_KITS: 'Puja Kits & Anushthan',
  ASTROLOGY: 'Astrology & Remedies',
  SPIRITUAL: 'Spiritual Accessories & Personalized',
  RUDRAKSHA: 'Rudraksha',
  IDOLS: 'Idols',
};
...
const CATEGORY_SLUGS = {
  ...
  'Daily Puja Essentials': 'daily-puja-essentials',
  'Gemstones & Rudraksha': 'gemstones--rudraksha',
  'Yantra & Idols': 'yantra--idols',
  'Puja Kits & Anushthan': 'puja-kits--anushthan',
  'Astrology & Remedies': 'astrology--remedies',
  'Spiritual Accessories & Personalized': 'spiritual-accessories--personalized',
  'Rudraksha': 'rudraksha',
  'Idols': 'idols',
};

// ✅ Product name se category assign karein
const getCategoryFromProduct = (product) => {
  const name = (product.name || '').toLowerCase();

  // Festival Collections
  if (name.includes('diwali') || name.includes('navratri') ||
      name.includes('janmashtami') || name.includes('holi') ||
      name.includes('karwa chauth') || name.includes('festival') ||
      name.includes('gulal') || name.includes('deepawali')) {
    return CATEGORY_NAMES.FESTIVAL;
  }

  // Daily Puja Essentials
  if (name.includes('dhoop') || name.includes('agarbatti') ||
      name.includes('ghee') || name.includes('diya') ||
      name.includes('incense') || name.includes('camphor') ||
      name.includes('kapoor') || name.includes('batti')) {
    return CATEGORY_NAMES.DAILY_PUJA;
  }

  // Gemstones & Rudraksha
  if (name.includes('gemstone') || name.includes('ruby') ||
      name.includes('emerald') || name.includes('sapphire') ||
      name.includes('pearl') || name.includes('pukhraj') ||
      name.includes('neelam') || name.includes('manik')) {
    return CATEGORY_NAMES.GEMSTONES;
  }

  // Yantra & Idols
  if (name.includes('yantra') || name.includes('idol') ||
      name.includes('murti') || name.includes('statue') ||
      name.includes('shivling')) {
    return CATEGORY_NAMES.YANTRA_IDOLS;
  }

  // Puja Kits & Anushthan
  if (name.includes('puja kit') || name.includes('anushthan') ||
      name.includes('havan') || name.includes('vrat') ||
      name.includes('satyanarayan') || name.includes('griha pravesh')) {
    return CATEGORY_NAMES.PUJA_KITS;
  }

  // Astrology & Remedies
  if (name.includes('nazar') || name.includes('remedy') ||
      name.includes('tortoise') || name.includes('horseshoe') ||
      name.includes('gomti') || name.includes('parad') ||
      name.includes('haldi')) {
    return CATEGORY_NAMES.ASTROLOGY;
  }

  // Rudraksha (separate)
  if (name.includes('rudraksha') || name.includes('mala')) {
    return CATEGORY_NAMES.RUDRAKSHA;
  }

  // Idols (separate)
  if (name.includes('idol') || name.includes('murti')) {
    return CATEGORY_NAMES.IDOLS;
  }

  // Default
  return CATEGORY_NAMES.SPIRITUAL;
};
...
const useProductStore = create((set, get) => ({
  ...
  fetchProducts: async () => {
    ...
    try {
      ...
      if (response.status === 401) {
        set({ products: [], loading: false, error: 'Authentication required' });
        return;
      }
        ...
        const productsWithCategory = data.products.map((product) => {
          ...
          return {
            ...
            category: {
              _id: product.category?._id || `manual_${categoryName}`,
              name: categoryName,
              slug: CATEGORY_SLUGS[categoryName] || categoryName.toLowerCase().replace(/ & /g, '--').replace(/ /g, '-'),
            },
          };
        });

        set({ products: productsWithCategory, loading: false, error: null });
      } else {
        set({ error: 'Invalid data structure from API', loading: false });
      }
    } catch (error) {
      console.error('❌ Fetch error:', error);
  ...
  getBestSellers: () => get().products.slice(0, 4),
  ...
  searchProducts: (searchTerm) => {
    const state = get();
    if (!searchTerm || searchTerm.trim() === '') return state.products;
    const term = searchTerm.trim().toLowerCase();
    return state.products.filter(
      (product) =>
        product.name.toLowerCase().includes(term) ||
  ...
  getProductsByPrice: (minPrice, maxPrice) =>
    get().products.filter((p) => p.price >= minPrice && p.price <= maxPrice),
  ...
  getRelatedProducts: (productId, limit = 4) => {
    ...
    if (!category) return [];
    return state.products
      .filter((p) => p._id !== productId && p.category?.name === category)
      .slice(0, limit);
  },
